...
class Chrom_Run():
    def __init__(self, Cellphone_id, Mitmproxy_port, Appium_port):
        ...
        ip = self.redis.get('{}'.format(Cellphone_id))
        if ip =="1":
            ...
            self.ip=ip
            self.redis.set('CW_IP', "1")
        self.cap = {
            "platformName": "Android",
            # "platformVersion": "5.1.1",  # 修改成匹配安卓版本，最好是版本低于7
            "platformVersion": "7.1.2",  # 修改成匹配安卓版本，最好是版本低于7
            ...
            # 允许传入chromeOptions参数
            "chromeOptions": {"args": ['--disable-notifications','--disable-search-geolocation-disclosure','--incognito','--disable-search-geolocation-disclosure',"user-agent={}".format(self.ua),'--proxy-server=http://{}'.format(self.ip)]}  # 允许传入chromeOptions参数
       }
    ...
    def _next(self, i):
        ...
        if v:
            ...
            el = self.driver.find_elements_by_xpath("//a")[random.randint(5, x)]
            Action = TouchActions(self.driver)
                        # el = i2.find_element_by_xpath("./..")
            TouchActions(self.driver).tap(el).perform()
            self.sub_hua_dong()
    ...
    def bak(self, name):
        ...
        for i2 in x:
            ...
            if "广告" in str(te):
                if "molixianhua" in str(te):
                    continue
                elif "xianhua214" in str(te):
                    continue
                elif "21cxhua" in str(te):
                    continue
                elif "pintai7" in str(te):
                    continue
                elif "pintai6" in str(te):
                    continue
                else:
                    click_list.append(i2)
        ...
        i2 = random.choice(click_list)
        te = i2.get_attribute('textContent')
        if "molixianhua" in te or "xianhua214" in te:
            return
        print(te)
        try:
            Action = TouchActions(self.driver)
    ...
    def start_mitmproxy_server(self):
        ...
        # return
        subprocess.Popen(cmd, stdout=open('C:\\Users\\asus\\Desktop\\vps3\\mitmproxy_log\\' + str(self.mitmproxy_port) + '.log', 'a'),
    ...
    def run(self):
        ...
        try:
            ...
            try:
                ...
                print(f)
                self.redis.set('{}'.format(Cellphone_id), "1")
                self.driver.quit()
                return
            ...
            with open("C:\\Users\\asus\\Desktop\\错误日志.log", "a", encoding="utf-8")as f:
                f.write(str(f))
        print("结束")
        self.driver.quit()
...
def run_app(Cellphone_id, Mitmproxy_port, Appium_port):
    ...
    try:
        ...
        subprocess.Popen(cmd, shell=True, stdout=open('C:\\Users\\asus\\Desktop\\vps3\\appium_log\\' + str(Appium_port) + '.log', 'a'),
                         stderr=subprocess.STDOUT)
        while True:
            try:
                print("Cellphone_id: ", Cellphone_id, "  Mitmproxy_port: ", Mitmproxy_port, "  Appium_port:", Appium_port)
                # 启动程序
                time.sleep(random.randint(1,4))
...
if __name__ == "__main__":
    ...
    for i in range(1,5):
        ...
        MAC = randomMAC()
        subprocess.Popen('Nox.exe -clone:Nox_{} -phoneNumber:{} -imei:{} -imsi:{} -mac:{}'.format(str(i),phone,IMEI,IMSI,MAC))
        time.sleep(10)
    time.sleep(30)
    ...
    print(Cellphone_ids)
    pools = Pool(processes=len(Cellphone_ids))
    # # 创建进程池
    while True:
        try:
